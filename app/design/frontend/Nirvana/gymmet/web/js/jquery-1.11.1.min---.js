...
! function(refresh) {
                ...
                u = function() {
                    for (var o = +new Date, n = 0, i = 0, c = 0; c < M.length; c++) {
                        var s = M[c],
                            d = o - s.start,
                            f = d >= g.animationTime,
                            m = f ? 1 : d / g.animationTime;
                        g.pulseAlgorithm && (m = h(m));
                        var p = s.x * m - s.lastX >> 0,
                            w = s.y * m - s.lastY >> 0;
                        n += p, i += w, s.lastX += p, s.lastY += w, f && (M.splice(c, 1), c--)
                    }
    ...
    function h(e) {
        return e >= 1 ? 1 : 0 >= e ? 0 : (1 == g.pulseNormalize && (g.pulseNormalize /= p(1)), p(e))
    }
    var w, v = {
            frameRate: 150,
            animationTime: 400,
